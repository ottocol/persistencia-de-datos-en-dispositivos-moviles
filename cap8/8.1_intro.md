...
Durante el proceso de desarrollo es normal que el modelo de datos vaya evolucionando: añadiremos/eliminaremos entidades, relaciones, atributos,… cambiaremos su nombre … No obstante hay un problema. Cada vez que se hace una modificación sobre el modelo de datos, el soporte persistente que ha creado Core Data (normalmente la base de datos SQLite) ya no sirve y hay que rehacerlo, y eso es un problema si hay datos guardados. Core Data detecta esta situación y aborta la aplicación generando un mensaje de error
