...
En este ejercicio vamos a crear una pequeña aplicación de gestión de tareas pendientes, en la que se puedan listar tareas y añadir tareas nuevas. Cada tarea tiene un `id` (entero, autonumérico), un `titulo` (cadena), una `prioridad` (entero) y un `vencimiento` (fecha). Las columnas están por este orden en la tabla. La base de datos ya está creada, y tú debes copiarla a tu proyecto como se explica a continuación.
...
- En la carpeta `archivos SQLite` de las plantillas hay unos cuantos recursos que debes copiar al proyecto
	- **Copia en el proyecto la base de datos** [`tareas.db`](plantillas/tareas.db) . **NO LO HAGAS ARRASTRANDO**, usa el menú `File > Add files to TareasSQLite...` y selecciona el archivo `tareas.db`. En el cuadro de diálogo de copia, pulsa sobre el boton `options` de la parte inferior y *asegúrate de que la casilla de `Copy files if needed` está marcada*. En caso contrario estás haciendo solo una referencia al archivo original, que se pierde si mueves el proyecto, y tampoco se sube al repositorio.
...
Ahora hay que configurar el proyecto para poder usar SQLite. **Añade la librería** `libsqlite3.tbd` y **crea** el *Bridging Header* según se explica en el apartado de [configuración de SQlite](2.1_intro_SQLite.md#config_sqlite) de los apuntes.
...
Si todo es correcto, en el *log* debe aparecer el mensaje "Base de datos *url_de_la_BD* abierta OK". **Una vez que sepas que funciona, quita la línea que has insertado** en el `viewDidLoad` para que no interfiera con el resto del ejercicio.
...
Para ir paso a paso, puedes implementar una versión inicial de `listarTareas` que solo saque de la BD el campo `titulo` (columna 1, de tipo cadena), y ponga el resto de campos a valores fijos y arbitrarios. Una vez lo tengas puedes probarlo en el  `MasterViewController`.
...
Deberían salir los títulos de 3 tareas distintas. Si esto va, ya sabes que la parte básica te funciona y ahora puedes añadir  en `listarTareas` el resto de campos: `id`, `prioridad` y `vencimiento`. Este último campo ten en cuenta que usa “tiempo UNIX”: número de segundos transcurridos desde el 1/1/1970.

### Mostrar las tareas en la tabla

Vamos a modificar el código del `viewDidLoad` para integrarlo mejor con la plantilla que ha generado Xcode. Fijate que lo que nosotros definimos como *lista* la plantilla lo define en la propiedad `objects` de la clase, y que es un array de objetos cualesquiera  (`[Any]()`). Cámbialo por un array de tareas, `[Tarea]()`), y cambia el código del apartado anterior para que use `self.objects` en vez de `lista`
...
```swift
objects.insert(Tarea(id:0, titulo:"Prueba", prioridad:1, vencimiento:Date()), at: 0)
```
...
> Quedaría mejor mostrar los datos de la tarea en la pantalla secundaria, pero sería demasiado tedioso, y nuestro objetivo es la parte de SQlite más que la interfaz.
...
@objc func nuevaTarea() {
    ...
    let crear = UIAlertAction(title: "Crear", style: .default) {
        ...
        if let prio = Int(alert.textFields![1].text!) {
            if let diasVenc = Double(alert.textFields![1].text!) {
               let venc = Date(timeIntervalSinceNow:24*60*60*diasVenc)
               let t = Tarea(id:0, titulo: tit, prioridad: prio, vencimiento: venc)
    ...
    alert.addTextField() { $0.placeholder = "Prioridad"}
...
Ahora al pulsar el botón `+` debería aparecer un alert para escribir los datos de la tarea. En esta versión simplificada no se chequea que la prioridad sea un número ni tampoco se puede poner una fecha de vencimiento día-mes-año, sino solo un número de días a partir de la fecha actual.